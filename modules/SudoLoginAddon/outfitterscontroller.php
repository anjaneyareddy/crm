...
require_once('modules/'.$currentModule.'/license/SudoLoginOutfittersLicense.php');

if($_REQUEST['method'] == 'validate') {
    SudoLoginOutfittersLicense::validate();
} else if($_REQUEST['method'] == 'change') {
    SudoLoginOutfittersLicense::change();
} else if($_REQUEST['method'] == 'add') {
    SudoLoginOutfittersLicense::add();
} else if($_REQUEST['method'] == 'test') {
    //optional param: user_id - test if a specific user has access to the add-on
    //Sugar 6: /index.php?module=SudoLoginAddon&action=outfitterscontroller&method=test&to_pdf=1
    //Sugar 7: #bwc/index.php?module=SudoLoginAddon&action=outfitterscontroller&method=test&to_pdf=1
    
    $user_id = null;
    if(!empty($_REQUEST['user_id'])) {
        $user_id = $_REQUEST['user_id'];
    }
    $validate_license = SudoLoginOutfittersLicense::isValid($currentModule,$user_id,true);
    ...
    if($validate_license !== true) {
        ...
        $validated = SudoLoginOutfittersLicense::doValidate($currentModule);
